...
export const tokenTable = sqliteTable(
  ...
  (table) => {
    return {
      ...
      constraints: [sql`CHECK (user_id IS NOT NULL OR admin_id IS NOT NULL)`],
    };
  }
);
...
export const subCategory = sqliteTable(
  ...
  (table) => {
    return {
      indexes: [{ columns: ["name"] }, { columns: ["category_id"] }],
    };
  }
);

// Sub Category Option
export const subCategoryOption = sqliteTable(
  "sub_category_option",
  {
    id: text().primaryKey().$default(nanoid),
    name: text().notNull(),
  ...
  (table) => {
    return {
      indexes: [{ columns: ["name"] }, { columns: ["sub_category_id"] }],
    };
  }
);
...
export const partner = sqliteTable(
  ...
  (table) => {
    return {
      indexes: [{ columns: ["user_id"] }, { columns: ["referral_code"] }],
    };
  }
);

// Partner Payment Info
export const partnerBank = sqliteTable(
  "partner_bank",
  {
    id: text().primaryKey().$default(nanoid),
    partner_id: text()
...
export const partnerIdProof = sqliteTable(
  "partner_id_proof",
  {
    id: text().primaryKey().$default(nanoid),
    partner_id: text()
    ...
    proof_type_id: integer()
      .$type(
        "ENUM",
        partnerIdProofTypes.map((type) => type.id)
      )
      .notNull(),
  ...
  (table) => {
    return {
      indexes: [{ columns: ["partner_id"] }, { columns: ["number"] }],
    };
  }
);
...
export const profile = sqliteTable(
  "profile",
  {
    ...
    sub_category_option_id: text()
      .notNull()
      .references(() => subCategoryOption.id, {
        onDelete: "CASCADE",
        onUpdate: "CASCADE",
      }),
...
// Profile Payment
export const profilePayment = sqliteTable(
  "profile_payment",
  {
    id: text().primaryKey().$default(nanoid),
    profile_id: text()
...
export const profileMedia = sqliteTable(
  ...
  (table) => {
    return {
      indexes: [{ columns: ["profile_id"] }, { columns: ["url"] }],
    };
  }
);
...
// License Type
export const licenseType = sqliteTable(
  "license_type",
  {
    id: text().primaryKey().$default(nanoid),
...
// Profile License
export const profileLicense = sqliteTable(
  "profile_license",
  {
    id: text().primaryKey().$default(nanoid),
    profile_id: text()
...
// Profile Tags
export const profileTag = sqliteTable(
  "profile_tag",
  {
    id: text().primaryKey().$default(nanoid),
    profile_id: text()
  ...
  (table) => {
    return {
      indexes: [{ columns: ["profile_id"] }, { columns: ["tag_id"] }],
    };
  }
);
...
export const testimonial = sqliteTable(
  ...
  (table) => {
    return {
      ...
      check: [gte("rating", 1), lte("rating", 5)],
    };
  }
);
